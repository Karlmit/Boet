...
function buildPrompt(text) {
  return [
    ...
    'sauce, a spice mix, a dressing, a filling, or a topping (this list is just examples — the',
    'right word always depends on what that specific group actually is in THIS recipe; never',
    'default to any one of them out of habit). Watch for a LATER step that refers back to such a',
    'group by name even though no earlier step used that name (e.g. one step blends several',
    'ingredients with no name given, and a later step says "stir in half the sauce" — those',
    'ingredients belong to a "Sås"/"sauce" tag because a later step named that mixture).',
    ...
    'the recipe\'s own language, e.g. ["Sås"] or ["Marinad","Fyllning"] for an ingredient used in',
    'both — the exact word always depends on what that group actually is, do not default to one',
    'word out of habit) — most ingredients get [] (no tag at all). Only tag ingredients that',
    'clearly belong to a named group; do not invent one just to have something to put there —',
    'most simple recipes have no such groups.',
    ...
    'few words in the recipe\'s own language that NAME the phase (e.g. "Gör såsen", "Marinera',
    'kycklingen"), SUMMARIZING everything from this step up to (but not including) the next',
    'titled step. The title must NOT just restate this one step\'s own instruction — e.g. a step',
    'that says "Preheat the oven to 200°C" does NOT get title "Preheat the oven to 200°C" or any',
    'close paraphrase of it; it needs a genuine section name for the phase that follows, or no',
    'title at all if there is nothing distinct to name. Every other step gets "title":null. Most',
    'recipes are simple enough that no step needs one at all — only add headers where the phase',
    'break is obvious, never one per step.',
...
function buildLinkingPrompt(ingredients, stepLines, unknownSectionIds, unknownTitleStepNumbers) {
  return [
    ...
    ...(unknownSectionIds.length > 0 ? [
      ...
      'sauce, a spice mix, a dressing, a filling, or a topping (this list is just examples — the',
      'right word always depends on what that specific group actually is in THIS recipe; never',
      'default to any one of them out of habit). Watch for a LATER step that refers back to such',
      'a group by name even though no earlier step used that name (e.g. one step blends several',
      'ingredients with no name given, and a later step says "stir in half the sauce" — those',
      'ingredients belong to a "Sås"/"sauce" tag because a later step named that mixture).',
      'Separately, some ingredients (often potatoes, rice, bread, a side salad) are only served',
      'ALONGSIDE the finished dish rather than cooked into it — tag those "Till servering"/"To',
      ...
      'array of short labels in the recipe\'s own language (usually just one, e.g. ["Sås"] or',
      '["Marinad"] — the exact word always depends on what that group actually is, do not default',
      'to one word out of habit) — an ingredient can have more than one. Leave an id out of',
      '"sections" entirely if it has no clear group; most ingredients will have none.',
    ] : []),
    ...(unknownTitleStepNumbers.length > 0 ? [
      ...
      'recipe\'s own language that NAME the phase (e.g. "Gör såsen", "Marinera kycklingen"),',
      'SUMMARIZING everything from that step up to (but not including) the next titled step. The',
      'title must NOT just restate that one step\'s own instruction — e.g. a step that says',
      '"Preheat the oven to 200°C" does NOT get a title that just repeats it; it needs a genuine',
      `section name for the phase that follows. For step numbers ${unknownTitleStepNumbers.join(', ')}`,
